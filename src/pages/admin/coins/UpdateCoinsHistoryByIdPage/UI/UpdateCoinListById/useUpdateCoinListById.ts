...
const useUpdateCoinListById = (userId: number) => {
    const [groupedCoins, setGroupedCoins] = useState<IGroupedCoins>({})

    useFirstLoadingAsync(async () => {
        const getCoinHistoryByIdResult = await CoinService.getHistoryUpdateCoinsById(userId)
        if (getCoinHistoryByIdResult.hasError()) {
            return AlertService.errorMessage(getCoinHistoryByIdResult.getError().errorMessage)
        }

        const sortedCoins = getCoinHistoryByIdResult.unwrap().sort((a: ICoinsDTO, b: ICoinsDTO) => {
...
export default useUpdateCoinListById
